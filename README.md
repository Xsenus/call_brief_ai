...
#### Если у вас уже есть ClientID и Client secret

Это еще не токен для нашего сервиса.

Что означает:

- `ClientID` — идентификатор приложения Яндекса
- `Client secret` — секрет приложения
- в `YANDEX_DISK_OAUTH_TOKEN` нужно записывать не `Client secret`, а именно готовый `access_token`

Как получить токен вручную:

1. Возьмите ваш `ClientID`.
2. Откройте в браузере ссылку такого вида:
...
```text
https://oauth.yandex.com/authorize?response_type=token&client_id=9f5a0e0916284d2eb30d518f04f93db1
```

3. Войдите в нужный Яндекс-аккаунт, если еще не вошли.
4. Подтвердите доступ приложению.
5. После этого Яндекс откроет страницу, где в адресе будет фрагмент вида:

```text
#access_token=AQAAAA...&expires_in=...
```

6. Скопируйте значение после `access_token=` и до следующего `&`.
7. Это значение и нужно записать в `YANDEX_DISK_OAUTH_TOKEN`.

Пример:

```dotenv
YANDEX_DISK_OAUTH_TOKEN=AQAAAA...
```

Что важно не перепутать:

- `ClientID` нужен для запроса авторизации
- `Client secret` не вставляется в `.env` нашего daemon-а
- в `.env` нужен именно итоговый `access_token`

Если `Client secret` уже был кому-то показан или попал в скриншот, безопаснее перевыпустить его в кабинете Яндекса.
